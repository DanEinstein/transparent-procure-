...
export default function Audit() {
  ...
  useEffect(() => {
    ...
    const fetchUrl = countyFilter
      ? `${baseUrl}?county=${encodeURIComponent(countyFilter)}`
      : baseUrl;
      ...
      .then((data) => {
        ...
        if (!countyFilter && parsedData.length > 0) {
          const uniqueCounties = [
            ...new Set(parsedData.map((t) => t.county).filter(Boolean)),
          ].sort();
          ...
          <tbody className="divide-y divide-slate-800">
            {loading ? (
              <tr>
                <td
                  colSpan="4"
                  className="text-center py-20 text-slate-500 font-mono text-xs uppercase tracking-widest"
                >
              ...
              <tr>
                ...
                >
                  No projects found for this county.
                </td>
              </tr>
            ) : (
              tenders.map((tender) => {
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider font-medium">
                          {tender.county || "Unknown County"} |{" "}
                          {tender.category || "General"}
                        </p>
                        {tender.is_demo_data && (
                          <span className="mt-1 w-max px-1.5 py-0.5 rounded text-[8px] font-black uppercase bg-primary/20 text-primary border border-primary/30">
                            DEMO DATA
                          </span>
                        )}
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span
                          className={`text-sm font-bold ${varianceRaw > 50 ? "text-rose-500" : "text-slate-300"}`}
                        >
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <p className="text-[9px] text-slate-600 italic font-bold">
                        KES
                      </p>
                    </td>
                  </tr>
